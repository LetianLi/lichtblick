...
describe("PlanningSceneExtension", () => {
  ...
  let mockServiceClient: jest.MockedFunction<
  ...
  describe("Transform Tree Integration", () => {
    ...
    it("sets up renderables for transform system correctly", () => {
      ...
      const renderable = extension.renderables.get("test_object");
      expect(renderable?.userData).toEqual(
    ...
    it("integrates with transform system via inherited startFrame", () => {
      ...
      expect(mockRenderer.transformTree.apply).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          // pose from userData (test data has origin pose)
  ...
  describe("Message Processing", () => {
    ...
    it("preserves object colors from scene", () => {
      ...
      scene.object_colors = [{ id: "colored_box", color: { r: 1.0, g: 0.0, b: 0.0, a: 0.8 } }];
      const handler = getMessageHandler();
  ...
  describe("Performance Optimizations", () => {
    ...
    it("skips unchanged ADD operations using object hashing", async () => {
      setupLayer();
      ...
      const second = createPlanningScene([createCollisionObject("hash_obj")]);
      second.is_diff = true;
  ...
  describe("State Persistence", () => {
    ...
    it("preserves object settings across remove/add cycles", () => {
      ...
      // Remove object
      const removeScene = createPlanningScene([
        createCollisionObject("persistent_object", CollisionObjectOperation.REMOVE),
      ]);
      handler!(createMessageEvent(removeScene));
  ...
  describe("Settings Management", () => {
    ...
    it("disables octomap setting by default", () => {
      ...
      const planningSceneNode = nodes.find(
        (node) => node.path.length === 2 && node.path[1] === "test-layer",
      );
